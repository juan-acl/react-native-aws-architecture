...
export const ConfirmSignUp = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      await confirmSignUp({
        username: emailParams,
        confirmationCode: emailCodeConfirmation,
      });
...
export const SignIn = createAsyncThunk(
  ...
  async ({ emailParams, passwordParams }: SignInPayload, thunkAPI) => {
    try {
      thunkAPI.dispatch(setIsLoading({ isLoading: true }));
      ...
      if (error instanceof AuthError) {
        switch (error.message) {
          case AuthFlowErrorTypes.USER_NOT_EXIST:
            ...
            thunkAPI.dispatch(authSlice.actions.setAuthError(true));
            break;
...
export const { setSignIn, setSignOut, setAuthError, setUserEmail } =
  authSlice.actions;
